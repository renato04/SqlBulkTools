...
namespace SqlBulkTools
{
    ...
    public class BulkInsert<T> : ITransaction
    {
        ...
        void ITransaction.CommitTransaction(SqlConnection connection)
        {
            if (!_list.Any())
            {
                return;
            ...
            using (SqlConnection conn = _helper.GetSqlConnection(connection))
            {
        ...
        /// <returns></returns>
        async Task ITransaction.CommitTransactionAsync(SqlConnection connection)
        {
            if (!_list.Any())
            {
                return;
